...
public class WebScanServiceImpl implements WebScanService{
	...
	public String getWords(String url) {
		...
		try {
			webReadHtmlUtility.findWords(countMap, url,wrd);
	        ...
	        operationOnStack(st,countMap,wrd);
			
	       
			
			js.put("count",  webReadHtmlUtility.sortElement(wrd, finalword));
	...
	private void puttingToStack(String url, Stack<String> st, Map<String, String> link) {
		for (Map.Entry<String,String> entry : link.entrySet()) { 
	    	...
	    	if(!entry.getValue().equals("#") && entry.getValue().toString()!=url && !entry.getValue().equals("javascript:void(0);") && entry.getValue()!="mailto:info@314e.com") {
	...
	private void operationOnStack(Stack<String> st, Map<String, Word> countMap, Map<String,Integer> wrd) throws IOException {
